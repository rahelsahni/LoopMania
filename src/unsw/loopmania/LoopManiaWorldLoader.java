...
public abstract class LoopManiaWorldLoader {
    ...
    public LoopManiaWorld load() {
        ...
        // load goals and goal string
        LogicalExpression goal = loadGoals(jsonGoals);
        world.setWorldGoal(goal);
        String goalString = loadGoalsString(jsonGoals);
        world.setWorldGoalString(goalString);
    ...
    /**
     * Constructs a string for the given goal
     * @param goals
     * @return String; null otherwise
     */
    public String loadGoalsString(JSONObject goals) {

        String goal = null;

        if (goals.getString("goal").equals("gold")) {

            int quantity = goals.getInt("quantity");
            goal = "Collect " + quantity + " Gold";
        }
        else if (goals.getString("goal").equals("experience")) {

            int quantity = goals.getInt("quantity");
            goal = "Gain " + quantity + " Xp";
        }
        else if (goals.getString("goal").equals("cycles")) {

            int quantity = goals.getInt("quantity");
            goal = "Complete " + quantity + " Cycles";
        }
        ...
        if (goals.getString("goal").equals("AND")) {
            ...
            String goalString1 = loadGoalsString(goal1);
            String goalString2 = loadGoalsString(goal2);

            goal = "(" + goalString1 + " and " + goalString2 + ")";
        }

        if (goals.getString("goal").equals("OR")) {
            ...
            goal = "(" + goalString1 + " or " + goalString2 + ")";
        }
    ...
    /**
     * create a LogicalExpression using the composite pattern
     * to create a tree structure for the given goal, using recursion
     * for subgoals
     * @param goals
     * @return LogicalExpression; null otherwise
     */
    public LogicalExpression loadGoals(JSONObject goals) {

        LogicalExpression goal = null;

        if (goals.getString("goal").equals("AND")) {

            goal = new AndCompositeGoal(goals.getJSONArray("subgoals"));
        }

        else if (goals.getString("goal").equals("OR")) {

            goal = new OrCompositeGoal(goals.getJSONArray("subgoals"));
        }
            ...
            if (goals.getString("goal").equals("gold")) {
                ...
                goal = new GoldGoal(goals.getInt("quantity"));
            }
            else if (goals.getString("goal").equals("experience")) {

                goal = new XpGoal(goals.getInt("quantity"));
            }
            else if (goals.getString("goal").equals("cycles")) {

                goal = new CycleGoal(goals.getInt("quantity"));
            }
            else if (goals.getString("goal").equals("bosses")) {
    ...
    private void loadEntity(LoopManiaWorld world, JSONObject currentJson, List<Pair<Integer, Integer>> orderedPath) {
        ...
        case "hero_castle":
            ...
            HeroCastle heroCastle = new HeroCastle(new SimpleIntegerProperty(character.getX()), new SimpleIntegerProperty(character.getY()));
